...
/**
 * Регистрация.
 * @param {text} pType - Код типа клиента
...
DECLARE
  ...
  arKeys        text[];
BEGIN
--  pType := lower(coalesce(pType, 'staffer.staff'));
  pType := 'staffer.staff';
  ...
  uArea := coalesce(GetAreaDefault(), GetArea(current_database()), GetAreaGuest());

  PERFORM SetSessionArea(uArea);

  uUserId := CreateUser(pUserName, pPassword, coalesce(NULLIF(trim(cn.short), ''), cn.name), pPhone, pEmail, cn.name, true, false, uArea);
  ...
  uClient := CreateStaff(null, CodeToType(pType, 'client'), null, pUserName, uUserId, pName, jPhone, jEmail, null, null, pDescription);

  FOR r IN SELECT st.id FROM Structure st WHERE st.code = 'hr-pm-main-soc.structure'
  LOOP
    FOR p IN SELECT ps.id FROM Position ps WHERE ps.structure = r.id
    LOOP
      FOR s IN SELECT st.id, st.userid FROM Staff st WHERE st.position = p.id
      LOOP
        vLabel := format('Регистрация сотрудника: %s', pUserName);
        PERFORM CreateNotice(s.userid, uClient, vLabel, 'staff');
        PERFORM CreateTask(uClient, GetType('system.task'), GetCalendar('default.calendar'), s.id, vLabel, false, interval '1 day', localtimestamp, localtimestamp + interval '1 day');
      END LOOP;
    END LOOP;
  END LOOP;
