...
AS $$
BEGIN
  RETURN CreateAddress(pParent, coalesce(pType, GetType('post.address')), pCode, pIndex, pCountry, pRegion, pDistrict, pCity, pSettlement, pStreet, pHouse, pBuilding, pStructure, pApartment, pAddress);
END;
$$ LANGUAGE plpgsql
   SECURITY DEFINER
...
BEGIN
  ...
  PERFORM EditAddress(uAddress, pParent, pType, pCode, pIndex, pCountry, pRegion, pDistrict, pCity, pSettlement, pStreet, pHouse, pBuilding, pStructure, pApartment, pAddress);
END;
$$ LANGUAGE plpgsql
   SECURITY DEFINER
   SET search_path = kernel, pg_temp;
