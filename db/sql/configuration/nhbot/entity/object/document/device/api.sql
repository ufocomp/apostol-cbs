...
AS $$
BEGIN
  RETURN CreateDevice(pParent, coalesce(pType, GetType('mobile.device')),
...
BEGIN
  ...
  PERFORM EditDevice(uId, pParent, pType, pModel, pClient, pIdentity, pVersion, pSerial, pAddress, piccid, pimsi, pLabel, pDescription);
END;
$$ LANGUAGE plpgsql
   SECURITY DEFINER
   SET search_path = kernel, pg_temp;
