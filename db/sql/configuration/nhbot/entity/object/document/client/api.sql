...
BEGIN
  ...
  uClient := CreateClient(pParent, coalesce(pType, GetType('physical.client')), pCode, pUserId, pName, pPhone, pEmail, pInfo, pCreation, pDescription);
...
BEGIN
  ...
  PERFORM EditClient(uClient, pParent, pType, pCode, pUserId, pName, pPhone, pEmail, pInfo, pCreation, pDescription);
END;
$$ LANGUAGE plpgsql
   SECURITY DEFINER
   SET search_path = kernel, pg_temp;
