...
#define exit_failure(msg) {                                 \
  if (GLog != nullptr)                                      \
    GLog->Error(APP_LOG_EMERG, 0, "Application: %s", msg);  \
  else                                                      \
...
namespace Apostol {
    ...
    namespace ShipSafety {
        ...
        void CNiceHashBot::ShowVersionInfo() {
        ...
        void CNiceHashBot::ParseCmdLine() {
        ...
        void CNiceHashBot::StartProcess() {
            if (Config()->Helper()) {
                m_ProcessType = ptHelper;
            }

            if (Config()->IniFile().ReadBool("process/NiceHash", "enable", true)) {
                AddProcess<CNiceHash>();
...
int main(int argc, char *argv[]) {
    ...
    CNiceHashBot NHBot(argc, argv);
    
    try
    {
        NHBot.Name() = APP_NAME;
        NHBot.Description() = APP_DESCRIPTION;
        NHBot.Version() = APP_VERSION;
        NHBot.Title() = APP_VER;

        NHBot.Run();

        exitcode = NHBot.ExitCode();
    }
